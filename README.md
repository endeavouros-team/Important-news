...
:--- | :--- | :---- | :---
2024.09.14 | `yay` does not work after the latest of pacman to 7.0.0, and here's a [workaround](https://forum.endeavouros.com/t/pacman-7-0-0-r3-g7736133-1-update). For `paru` we unfortunately need to wait for a solution.<br>Update 2024-09-15: the new version (12.3.5-2) of `yay` is OK now.<br>Update-2 (2024-09-15): the latest version of `yay` is working again. A temporary workaround for `paru` is to use `paru-git` until upstream `paru` gets fixed. | yay, paru | pacman 7.0.0
